...
float half_to_float(uint h) {
	...
	uint  bits				  = two_w < denormalized_cutoff ? floatBitsToUint(denormalized_value)
...
float f32_to_f16_to_f32(float f) {
	...
	uint  result_bits =
		two_w < denorm_cutoff ? floatBitsToUint(denormalized) : floatBitsToUint(normalized);
...
uint float_to_half(float f) {
	...
	if (exp_field == 0u || exp_field < 113u)
		return sign;
	if (exp_field >= 142u)
		return sign | 0x7C00u;
