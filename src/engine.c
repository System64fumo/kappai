...
static status_code warmup_run(context *c, const char *system) {
	...
	status_code rc = chat_template_warmup(&c->chat, &c->tok, system);
	if (rc != OK)
		return rc == ERR_INVALID_ARG ? ERR_INVALID_ARG : OK;

	int n = c->chat.n_static_prefix_ids;
	if (n == 0)
		return OK;

	context_monitor_send_start(c);
	prefill_result pf =
		context_prefill_tokens(c, c->chat.static_prefix_ids, n, "warmup", true);
	if (pf.rc < 0)
		return ERR_INTERNAL;

	c->warmup_done = true;

	DEBUG("warmup: prefilled %d pre-cached tokens", n);
	return OK;
}
